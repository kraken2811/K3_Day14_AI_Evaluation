...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Score the answer against each rubric criterion. Return JSON with "
            "criterion names mapped to numeric scores from 0 to 1.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_lines}"
        )
        ...
        try:
            ...
            if isinstance(parsed, dict) and isinstance(parsed.get("scores"), dict):
                ...
                scores = {}
            normalized = {
                criterion: _clamp(float(scores.get(criterion, 0.5)))
                for criterion in rubric
            }
        except (TypeError, ValueError):
            normalized = {criterion: 0.5 for criterion in rubric}
        return {"scores": normalized, "reasoning": raw}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            ...
            "severity_bias": average < 0.3 if all_scores else False,
        }
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        for metric in metrics:
            new_avg = metric_avg(new_results, metric)
            baseline_avg = metric_avg(baseline_results, metric)
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        metric = min(scores, key=scores.get)
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggestion = (
                ...
                else "Review failed trace and add targeted fix"
            )
            rows.append(
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0):
            suggestions.append("Add grounding checks that reject claims not supported by retrieved context")
        if categories.get("irrelevant", 0) or categories.get("off_topic", 0):
            suggestions.append("Tighten the answer prompt to restate the user intent before composing the response")
        if categories.get("incomplete", 0):
            suggestions.append("Increase top-k or chunk coverage for policy questions with multiple conditions")
        if any((failure.context_recall or 0.0) < 0.5 for failure in failures):
            suggestions.append("Improve retrieval queries with domain synonyms and metadata-aware source boosts")
        if any((failure.context_precision or 1.0) < 0.5 for failure in failures):
            suggestions.append("Add lexical or cross-encoder reranking before generation")
        suggestions.append("Add the recurring failed cases to the regression benchmark with a 0.05 metric-drop gate")

        deduped = list(dict.fromkeys(suggestions))
        while len(deduped) < 3:
            deduped.append("Review retrieved chunks and expected-answer coverage for each failed case")
        return deduped
